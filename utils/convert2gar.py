...
def augustus_to_gar(work_path, program, add_nuc, test, rem_file=False):
    """
    ...
    if test:
        try:
            output = os.path.join(os.getcwd(), "Test", "Predictions", "Exon_maps", f"{program}", "150bp")
            os.makedirs(output, exist_ok=True)
            path_prediction = os.path.join(os.getcwd(), "Test", "Predictions", f"{program}", "150bp")
            ...
            for file in liste_file:
                with open(os.path.join(path_prediction, file), "r") as file_1:
                    for ligne in file_1:
                        if "# (none)" in ligne:
                            nbr_file_without_prediction += 1
                            if rem_file is True:
                                os.remove(os.path.join(path_prediction, file))
                            else:
                                pass
                            
                #print(os.path.join(path_prediction, file))

                with open(os.path.join(path_prediction, file), "r") as file_X:
                    ...
                    for k, v in dico_longueur_gene.items():
                        ...
                        #print(longueur_sequence, add_nuc)
                    ...
                    if best_transcrit == "None":
                        ...
                        with open(os.path.join(output, file.replace(".fasta", "") + ".gar"), "w") as file_E:
                        ...
                        with open(os.path.join(path_prediction, file), "r") as file_X2:
                            liste_cds = []
                            liste_exon = []
                            start_codon = "None"
                            stop_codon = "None"
                            ...
                            #File CDS
                            with open(os.path.join(output, file.replace(".fasta", "") + "_cds.gar"), "w") as file_E:
                            ...
                            if len(liste_myformat_exon) == 0:
                                ...
                                with open(os.path.join(output, file.replace(".fasta", "") + "_exon.gar"), "w") as file_E:
        ...
        try:
            if add_nuc != 150:
                output = os.path.join(work_path, "Predictions", "Exon_maps", f"{program}", f"{add_nuc}Kb")
                os.makedirs(output, exist_ok=True)

                path_prediction = os.path(work_path, "Predictions", f"{program}", f"{add_nuc}Kb")
                liste_file = os.listdir(path_prediction)
                add_nuc = int(add_nuc) * 1000

            else:
                add_nuc = int(add_nuc)
                output = os.path.join(work_path, "Predictions", "Exon_maps", f"{program}", "150bp")
                os.makedirs(output, exist_ok=True)
                path_prediction = os.path.join(work_path, "Predictions", f"{program}", "150bp")
                liste_file = os.listdir(path_prediction)
...
def helixer_to_gar(work_path, program, add_nuc, test, rem_file=False):
    """
    Converts prediction file from helixer to .gar file
    :param work_path: chemin du dossier de travail (root)
    :param program: Name of the predictor
    :param add_nuc: Number of flanked DNA
    :param rem_file: if True, remove files without prediction
    ...
    if test:
        try:
            ...
            for file in liste_file:
                ...
                size = os.path.getsize(os.path.join(path_prediction, file))
                if size < 200:
                    activator = True
                    nbr_file_without_prediction += 1
                    if rem_file is True:
                ...
                # Predictions ok
                with open(os.path.join(path_prediction, file), "r") as file_X:
                    id_uniprot = file.replace("helixer_", "").replace(".fasta", "")
                    ...
                    for ligne in file_X:
                        ...
                        if "CDS\t" in ligne:
                            ...
                            num_transcrit = ligne[8].split("|")[-1].split(".")[0]
                    ...
                    print(">>> The gene of interest for : " + str(file).replace("helixer_", "").replace(".fasta", "") +
                        " is : " + str(best_transcrit))

                    if best_transcrit == "None":
                        print(">>> No good prediction in " + file)
                        with open(os.path.join(output, file.replace(".fasta", "") + ".gar"), "w") as file_E:
                            file_E.write(f"# Transcrit_{best_transcrit};{id_uniprot};PREDICTED_by_Helixer\n")
                            file_E.write("> 0 exon\n\n")
                        ...
                        with open(os.path.join(path_prediction, file), "r") as file_X2:
                            ...
                            for i, ligne in enumerate(file_X2):
                                ...
                                try:
                                    num_tran = ligne[8].split("|")[-1].split(".")[0]
                                except IndexError:
                                    num_tran = ""
                                    pass
        ...
        try:
            ...
            if add_nuc != 150:
                ...
                path_prediction = os.path.join(work_path, "Predictions", f"{program}", f"{add_nuc}Kb")
                liste_file = os.listdir(path_prediction)
                add_nuc = int(add_nuc) * 1000
                ...
                output = os.path.join(work_path, "Predictions", "Exon_maps", f"{program}", "150bp")
                os.makedirs(output, exist_ok=True)
                path_prediction = os.path.join(work_path, f"Predictions", f"{program}", "150bp")
                liste_file = os.listdir(path_prediction)
...
def hmmgene_to_gar(work_path, program, add_nuc, rem_file=False):
    """
    Converts prediction file from helixer to .gar file
    :param work_path: chemin du dossier de travail (root)
    :param program: Name of the predictor
    ...
    for file in liste_file:
        ...
        # Predictions ok
        with open(path_prediction + file, "r") as file_X:
            id_uniprot = file.replace("hmmgene_", "").replace(".fasta", "")
            ...
            print(">>> The gene of interest for : " + str(file).replace("hmmgene_", "").replace(".fasta", "") +
                  " is : " + str(best_transcrit))

            if best_transcrit == "None":
                print(">>> No good prediction in " + file)
                with open(output + file.replace(".fasta", "") + ".gar", "w") as file_E:
                    file_E.write(f"# Transcrit_{best_transcrit};{id_uniprot};PREDICTED_by_HMMGENE\n")
                    file_E.write("> 0 exon\n\n")
...
if __name__ == '__main__':
    ...
    _nuc = [150, 2, 4, 6, 8, 10]
    for add_nuc in _nuc:
        augustus_to_gar(workpath, "augustus", add_nuc, True)
        ...
        helixer_to_gar(workpath, "helixer", add_nuc, True)
        #hmmgene_to_gar(workpath, "hmmgene", add_nuc)
