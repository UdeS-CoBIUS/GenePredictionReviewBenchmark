...
def create_prediction(work_path, soft, add_nuc, file):
    ...
    if soft == "augustus":
        ...
        if add_nuc == 150:
            file_cds = Path(os.path.join(work_path, "Predictions", "Exon_maps", f"{soft}", "150bp", f"{file.replace('_exon', '_cds')}"))
            file_exon = Path(os.path.join(work_path, "Predictions", "Exon_maps", f"{soft}", "150bp", f"{file.replace('_cds','_exon')}"))
        else:
            file_cds = Path(os.path.join(work_path, "Predictions", "Exon_maps", f"{soft}", f"{add_nuc}Kb", f"{file.replace('_exon', '_cds')}"))
            file_exon = Path(os.path.join(work_path, "Predictions", "Exon_maps", f"{soft}", f"{add_nuc}Kb", f"{file.replace('_cds','_exon')}"))
        ...
        if add_nuc == 150:
            file = os.path.join(work_path, "Predictions", "Exon_maps", f"{soft}", "150bp", f"{file}")
        else:
            file = os.path.join(work_path, "Predictions", "Exon_maps", f"{soft}", f"{add_nuc}Kb", "{file}")
...
def create_reference(work_path, file, soft):
    ...
    #print(file, soft)
    #print(os.path.join(work_path, "References", "Exon_maps", f"{file.replace(soft+'_', '').replace('_cds','').replace('_exon','').replace('.gff3.gar', '.gar')}"))
    with open(os.path.join(work_path, "References", "Exon_maps", f"{file.replace(soft+'_', '').replace('_cds','').replace('_exon','').replace('.gff3.gar', '.gar')}") , "r") as file_R1:
        for ligne in file_R1:
            if "Exon_" in ligne:
                ligne = ligne.strip().split("\t")
...
def comparison(work_path, soft, add_nuc, file, test_path):
    """
    ...
    """
    if test_path != None:
        work_path_test = test_path
        output_dir = os.path.join(work_path_test, "Resultats", "exon", f"{soft}", "150bp")
        

        os.makedirs(os.path.join(output_dir, "Details"), exist_ok=True)
        os.makedirs(os.path.join(output_dir, "Calculs"), exist_ok=True)
        os.makedirs(os.path.join(output_dir, "Boundaries"), exist_ok=True)

        # generate the prediction structure
        if soft == "augustus":
            dict_exon_pred_exon = create_prediction(work_path_test, soft, add_nuc, file)[0]
            dict_exon_pred_cds = create_prediction(work_path_test, soft, add_nuc, file)[1]

        #elif soft == "genscan":
            #dict_exon_pred = create_prediction(work_path, soft, add_nuc, file)
        elif soft == "helixer":
            dict_exon_pred = create_prediction(work_path_test, soft, add_nuc, file)
            ...
            print(">>> Error in program name, please choose : augustus, geneid, glimmer or snap")
            exit()
        ...
        with open(os.path.join(output_dir, "Details", f"{file.replace('.gar', '.csv')}"), "a") as file_details:

            file_details.write("Correct Ref;Correct Pred;MissingExon;WrongExon;OutExon;Good_Donnor;Good_Acceptor\n")

            with open(os.path.join(output_dir, "Calculs", f"{file.replace('.gar', '.csv')}"), "a") as file_calcul:
                ...
                with open(os.path.join(output_dir, "Boundaries", f"{file.replace('.gar', '.csv')}"), "a") as file_limite:
        ...
        if soft == "augustus":
            try:
                if score_cds >= score_exon:
                    os.remove(os.path.join(output_dir,  "Details", f"{file.replace('_cds','').replace('_exon','').replace('.gar', '')}_exon.csv"))
                    os.remove(os.path.join(output_dir, "Calculs", f"{file.replace('_cds','').replace('_exon','').replace('.gar', '')}_exon.csv"))
                    os.remove(os.path.join(output_dir, "Boundaries", f"{file.replace('_cds','').replace('_exon','').replace('.gar', '')}_exon.csv"))

                else:
                    os.remove(os.path.join(output_dir, "Details", f"{file.replace('_cds','').replace('_exon','').replace('.gar', '')}_cds.csv"))
                    os.remove(os.path.join(output_dir, "Calculs", f"{file.replace('_cds','').replace('_exon','').replace('.gar', '')}_cds.csv"))
                    os.remove(os.path.join(output_dir, "Boundaries", f"{file.replace('_cds','').replace('_exon','').replace('.gar', '')}_cds.csv"))

            except FileNotFoundError:
                pass
    else:
        if add_nuc == 150:
            output_dir = os.path.join(work_path, "Resultats", "exon", f"{soft}", "150bp")
        else:
            output_dir = os.path.join(work_path, "Resultats", "exon", f"{soft}", f"{str(add_nuc)}Kb")
...
def average(work_path, soft, add_nuc, file):
    ...
    """
    if add_nuc == 150:
        path = os.path.join(work_path, "Resultats", "exon", f"{soft}", "150bp", "Calculs", f"{file.replace('.gar', '.csv')}")
    else:
        path = os.path.join(work_path, "Resultats", "exon", f"{soft}", f"{add_nuc}Kb", "Calculs", f"{file.replace('.gar', '.csv')}")
    print(path)
...
def calcul_average(work_path, soft, add_nuc, liste_gene):
    ...
    with open(os.path.join(output_dir, "results.csv"), "w") as file_W2:
...
def count_number_good_start_stop(workpath, soft, add_nuc, seq_type, test_path):
    if test_path != None:
        if add_nuc == 150:
        ...
        #liste_validated = []
        #with open(os.path.join("g3po_main", "Sequence_types", "g3po", f"{seq_type}.seq"), "r") as file_R0:
        ...
        nbr_5prime = 0
        nbr_3prime = 0
        #print(liste_validated)
        liste_validated = ['B3M138_DROAN']
        for file in liste_validated:
            if soft == "augustus":
                try:
                    path = Path(os.path.join(f"{test_path}", f"{path_b}{soft}_{file}_exon.csv"))
                    open(path)
                except FileNotFoundError:
                    try:
                        path = Path(os.path.join(f"{test_path}", f"{path_b}{soft}_{file}_cds.csv"))
                        open(path)
                    except FileNotFoundError:
                        path = Path(os.path.join(f"{test_path}", f"{path_b}{soft}_{file}.csv"))

            else:
                path = Path(os.path.join(f"{test_path}", f"{path_b}{soft}_{file}.gff3.csv"))

            #with open(f"{path}".replace('.gff3.csv', '.csv'), "r") as file_R1:
            #with open(f"{path}".replace('.csv', '.gff3.csv'), "r") as file_R1:
            #if path.exists :
        ...
        for file in liste_validated:
            if soft == "augustus":
                try:
                    path = Path(os.path.join(f"{workpath}", f"{path_b}{soft}_{file}_exon.csv"))
                    open(path)
                except FileNotFoundError:
                    try:
                        path = Path(os.path.join(f"{workpath}", f"{path_b}{soft}_{file}_cds.csv"))
                        open(path)
                    except FileNotFoundError:
                        path = Path(os.path.join(f"{workpath}", f"{path_b}{soft}_{file}.csv"))
...
def main_script_exon(workpath, l_soft, test):
    seq_type = "Confirmed"
    _nuc = [150, 2, 4, 6, 8, 10]
    #l_soft = ["augustus", "glimmer", "helixer", "snap"]
    ...
    if test:
        try:
            for soft in l_soft:
                add_nuc = 150
                ref_dir = os.path.join(os.path.join(os.getcwd(), "Test"), "Predictions", "Exon_maps", f"{soft}", "150bp")
                
                print(ref_dir)
                liste_gene = os.listdir(ref_dir)

                for file in tqdm(liste_gene):
                    comparison(workpath, soft, add_nuc, file, os.path.join(os.getcwd(), "Test"))

                calcul_average(os.path.join(os.getcwd(), "Test"), soft, add_nuc, liste_gene)
                count_number_good_start_stop(workpath, soft, add_nuc, seq_type, os.path.join(os.getcwd(), "Test"))
            return True
        except:
            return False
    else:
        try:
            for soft in l_soft:
                for add_nuc in _nuc:
                    ...
                    if add_nuc == 150:
                        ref_dir = os.path.join(workpath, "Predictions", "Exon_maps", f"{soft}", "150bp")
                    else:
                        ref_dir = os.path.join(workpath, "Predictions", "Exon_maps", f"{soft}", f"{add_nuc}Kb")

                    liste_gene = os.listdir(ref_dir)

                    for file in tqdm(liste_gene):
                        comparison(workpath, soft, add_nuc, file, None)

                    calcul_average(workpath, soft, add_nuc, liste_gene)
                    count_number_good_start_stop(workpath, soft, add_nuc, seq_type, None)
            return True
        except:
            return False
...
if __name__ == '__main__':
    ...
    main_script_exon(workpath, l_soft, False)
